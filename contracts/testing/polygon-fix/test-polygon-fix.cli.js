...
historyAmounts = JSON.parse(
    fs.readFileSync('./testing/polygon-fix/amounts.json'),
);
historyAmounts.length;
historyTimestamps = JSON.parse(
    fs.readFileSync('./testing/polygon-fix/timestamps.json'),
);
...
balancesAfter.map(fe);

deltas = _.zipWith(balancesBefore, balancesAfter, (b, a) => a.sub(b));
showBalances(deltas);
