...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    console.log('Adding zSwap key to panther pool');
    const circuitId = getContractEnvAddress(hre, 'VK_ZSWAP');

    const tx = await pantherPool.updateCircuitId(0x106, circuitId);
...
func.tags = ['add-zswap-key-to-pool', 'protocol'];
