...
// @notice The binary Merkle tree populated with zero leaf values
abstract contract SixLevelZeroTree {
    // @dev Number of levels in a tree excluding the root level
    // solhint-disable-next-line var-name-mixedcase
    uint256 internal constant TREE_DEPTH = 6;

    // @param level From 0 for the leaf up to TREE_DEPTH for the root
    function getZeroNodeAtLevel(uint256 level) internal pure returns (bytes32) {
        require(level < TREE_DEPTH + 1, "SZT:E1");
