...
describe.only('ZAccountsRegistry', function () {
    ...
    const eip712Salt = ethers.utils.keccak256(
    ...
    beforeEach(async () => {
        ...
        const BabyJJ = await ethers.getContractFactory('BabyJubJub');
        const babyJJ = await BabyJJ.deploy();
        ...
        const ZAccountsRegistry = await ethers.getContractFactory(
            'MockZAccountsRegistry',
            {
                libraries: {
                    ...
                    BabyJubJub: babyJJ.address,
                },
            },
        );
        // console.log(
        //     'Formatted Name',
        //     ethers.utils.formatBytes32String('ZACCOUNT_REGISTRACTION'),
        // );
        // console.log('Formatted Version', ethers.utils.formatBytes32String('1'));
    ...
    async function registerZAccount(
        pubRootSpendingKey: any,
        pubReadingKey: any,
        signer: SignerWithAddress,
        expectedZAccountId = 1,
    ) {
        const {v, r, s} = await genSignature(
            '0x829a1faf9428f2e0d10e923ddb0001ce2e3748cbd8c844145967992212f5a82d',
            '0x829a1faf9428f2e0d10e923ddb0001ce2e3748cbd8c844145967992212f5a82d',
            signer,
        );
        ...
        //     expectedZAccountId,
        //     zAccountVersion,
        //     [pubRootSpendingKey.x, pubRootSpendingKey.y],
        //     [pubReadingKey.x, pubReadingKey.y],
        // ]);
    }

    describe.only('#registerZAccount', () => {
        describe.only('Success', () => {
            it('should register zAccount when signature is correct', async () => {
                const pubRootSpendingKey = {
                ...
                console.log(await zAccountsRegistry.zAccounts(user.address));
                // expect(
                //     await zAccountsRegistry.zAccounts(user.address),
