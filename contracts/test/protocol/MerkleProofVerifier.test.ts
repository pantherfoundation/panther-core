...
describe('MerkleProofVerifier', () => {
    ...
    describe('internal `testVerifyMerkleProof` method - by using circom zkp-input test values', function () {
        describe('a call inserting 3 zero leaves & checking proof for each leaf', function () {
            ...
            const pubKey = [
                ...
                BigInt(
                    '2792662591747231738854329419102915533513463924144922287150280827153219249810',
                ),
            ];
            const commitment0 = poseidon([
                pubKey[0],
                pubKey[1],
                amountsOut[0],
                token,
            ...
            const merkleRoot = toBytes32(
                '10650828051129756317708141452089125851926509526003232203604419064374393231061',
            );

            const pathElements0 = [
                toBytes32(
                    '70575835294174784547258244826603579894295126084098374754150034768326978226',
                ),
                toBytes32(
                    '21165498949491829088726564027049018044359747064678034716482327251128763739323',
                ),
                toBytes32(
                    '5317387130258456662214331362918410991734007599705406860481038345552731150762',
                ),
                toBytes32(
                    '5301900180746108365834837840355741695167403565517259206503735319173783315742',
                ),
                toBytes32(
                    '19759440382600727929415049642887307143518671081639244670052489500787514850212',
                ),
                toBytes32(
                    '11575399251628151734428362828441614938772848828475906566857213866326592241179',
                ),
                toBytes32(
                    '6632555919090241659299800894218068745568431736196896666697681740099319754273',
                ),
                toBytes32(
                    '2313232035512824863888346564211238648697583940443483502600731472911335817854',
                ),
                toBytes32(
                    '12219166190744012474665556054784140979314676975916090596913570678231824844496',
                ),
                toBytes32(
                    '16146864604902996392229526390577377437180881860230124064882884440248322100339',
                ),
                toBytes32(
                    '6883543445806624803603297055410892317599264946303553983246148642156945721809',
                ),
                toBytes32(
                    '11376031557295681140127084012245938798408060888509383225192187436273860950878',
                ),
                toBytes32(
                    '13241605803954237324747758640385138335781780544452364878098724458062976117242',
                ),
                toBytes32(
                    '17855149516804167337625231993818327714748909580849949294952537831754058414670',
                ),
                toBytes32(
                    '5150255556564484319136269061916843962561348275990403501481125286754601797805',
                ),
                toBytes32(
                    '6987786980040962217323608240860512602136308242543772977912408457104385595406',
                ),
            ] as PathElementsType;

            const pathElements1 = [
                toBytes32(
                    '5001742625244953632730801981278686902609014698786426456727933168831153597234',
                ),
                toBytes32(
                    '21165498949491829088726564027049018044359747064678034716482327251128763739323',
                ),
    ...
    describe('verify proof using zero panther-core tree & solidity verifier', function () {
        before(async () => {
            snapshot = await takeSnapshot();
        });
        ...
        describe('should be equality between ts & solidity path elements & proof will success', function () {
            ...
            const ShouldBeMerklePathElementsAfterFirstInsert = [
                ...
                BigInt(
                    '3349047423219193406330965173426204517756040645871630854057691440868894250982',
                ),
                BigInt(
                    '18389954371877325743937615564349876315640427734567075272665046346265626136419',
                ),
                BigInt(
                    '3821922445747924499025173562938580174383118354164359337630642212084359151964',
                ),
                BigInt(
                    '15935733969631511252102568819760944197418770481327957873988205677660925018528',
                ),
                BigInt(
                    '11782991327328543086851214586786607762143799684091548387988272710726371549961',
                ),
                BigInt(
                    '20296808824597379678225500535446241165197388668932210796624301020410505806483',
                ),
                BigInt(
                    '4173461319953077503036196915980451538453535748888760632593364006273103304132',
                ),
                BigInt(
                    '5766550159403151835612862031619173244724183903452415224168581364310081162759',
                ),
                BigInt(
                    '10719667445803564685804016390777214089338112164281015443530526835727343022767',
                ),
                BigInt(
                    '21349090590431709965480677812339735277896174812144673690644796244835835356674',
                ),
                BigInt(
                    '19531707066138634990416163973328796061422245663290449768207249753220005371133',
                ),
                BigInt(
                    '13000046769163827723557373669699328816629124803440350859991091474655812341048',
                ),
                BigInt(
                    '8951578653298612361448433248556484464983144095284075554880538299310385645682',
                ),
                BigInt(
                    '7870690898942382169582441685508490691047003383534923922466972436590775853570',
                ), // ROOT
            ];
            ...
            const ShouldBeMerklePathElementsAfterSecondInsert = [
                ...
                BigInt(
                    '5080802032616611841695934472369605256187370514682593051886813285782187880244',
                ), // ROOT
            ];
            ...
            const ShouldBeMerklePathElementsAfterThirdInsert = [
                ...
                BigInt(
                    '6593769061588505652796652368972428248449904784599508005290567407050120675396',
                ),
                BigInt(
                    '18389954371877325743937615564349876315640427734567075272665046346265626136419',
                ),
                ...
                BigInt(
                    '12639523502428448254562583832651707893831215707918737401127830898440049948195',
                ), // ROOT
            ];
        ...
        describe('Crypto infra tests', function () {
            describe('CipherTest encryption decryption', function () {
                ...
                const R_packed_from_chain = cipherTextMessageV1.slice(
                    16,
                    16 + 32,
                );
                ...
                //    await trees.GeneratePublicSpendingKey(s);
                //});
            });

            describe('Spend & Random Keys generation should be equal to solidity side', function () {
            ...
            describe('Spend & Random Keys generation', () => {
                ...
                const SpenderDerivedPubKeyOnRecipientSide =
                    babyjub.mulPointEscalar(babyjub.Base8, sr); // S = sB S' = srB
                ...
                describe('should be equal', () => {
                    it('SpenderDerivedPubKeyOnRecipientSide == SpenderDerivedPubKeyOnSenderSide', () => {
                        expect(
                            SpenderDerivedPubKeyOnRecipientSide[0].toString(),
                        ).equal(SpenderDerivedPubKeyOnSenderSide[0].toString());
                        expect(
                            SpenderDerivedPubKeyOnRecipientSide[1].toString(),
                        ).equal(SpenderDerivedPubKeyOnSenderSide[1].toString());
                    });
                });
            });
        });
