...
circom_compile_v1_extended_step0 () {
    ...
    ${CIRCOM} --r1cs --wasm --sym -o compiled/ circuits/mainZSwapV1.circom;
}
...
snarkjs_r1cs_info_v1_extended_step1 (){
    ...
    ${SNARKJS} r1cs info compiled/mainZSwapV1.r1cs;
}

snarkjs_export_r1cs_json_step2 () {
    echo "*** snarkjs_export_r1cs_json_step2 ***";
    ${SNARKJS} r1cs export json compiled/mainZSwapV1.r1cs compiled/zswap.json
}

snarkjs_pseudo_setup_groth16_step3 () {
    echo "*** snarkjs_pseudo_setup_groth16_step3 ***";
    ${SNARKJS} groth16 setup compiled/mainZSwapV1.r1cs ./powersOfTau28_hez_final_18.ptau compiled/zswap_0000.zkey
}

snarkjs_phase2_contribute_1_step4 () {
    echo "*** snarkjs_phase2_contribute_1_step4 ***";
    ${SNARKJS} zkey contribute compiled/zswap_0000.zkey compiled/zswap_0001.zkey --name="1st Contributor Name" -v
}

snarkjs_phase2_contribute_2_step5 () {
    echo "*** snarkjs_phase2_contribute_2_step5 ***";
    ${SNARKJS} zkey contribute compiled/zswap_0001.zkey compiled/zswap_0002.zkey --name="2st Contributor Name" -v
}

snarkjs_phase2_contribute_3_step6 () {
    echo "*** snarkjs_phase2_contribute_3_step6 ***";
    ${SNARKJS} zkey contribute compiled/zswap_0002.zkey compiled/zswap_0003.zkey --name="3st Contributor Name" -v
}

snarkjs_phase2_zkey_verify_step7 () {
    echo "*** snarkjs_phase2_zkey_verify_step7 ***";
    ${SNARKJS} zkey verify compiled/mainZSwapV1.r1cs ./powersOfTau28_hez_final_18.ptau compiled/zswap_0003.zkey
}

snarkjs_phase2_apply_random_beacon_step8 () {
    echo "*** snarkjs_phase2_apply_random_beacon_step8 ***";
    ${SNARKJS} zkey beacon compiled/zswap_0003.zkey compiled/zswap_final.zkey 0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f 10 -n="Final Beacon phase2"
}

snarkjs_final_zkey_verify_step9 () {
    echo "*** snarkjs_final_zkey_verify_step9 ***";
    ${SNARKJS} zkey verify compiled/mainZSwapV1.r1cs ./powersOfTau28_hez_final_18.ptau compiled/zswap_final.zkey
}

snarkjs_export_verification_key_step10 () {
    echo "*** snarkjs_export_verification_key_step10 ***";
    ${SNARKJS} zkey export verificationkey compiled/zswap_final.zkey compiled/zswap_verification_key.json
}

snarkjs_create_solidity_verifier_step11 () {
    echo "*** snarkjs_create_solidity_verifier_step11 ***";
    ${SNARKJS} zkey export solidityverifier compiled/zswap_final.zkey compiled/zswap_final_verifier.sol
}
